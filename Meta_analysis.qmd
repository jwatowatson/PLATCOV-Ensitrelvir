...
itt_population_all = read.csv('ITT_population_meta.csv')
...
```{r}
ff = list.files('Rout/Meta_linear/', pattern = paste0(intervention,'.RData'),)
...
print(G3_trt_effect + ggtitle("Estimated treatment effects"))
```

```{r}
f_name <- paste("Plots/hl_trt_eff_meta_analysis.pdf")

pdf(f_name, width = 6, height = 5)
G3_trt_effect + ggtitle("Estimated treatment effects relative to \nno study drug arm")
dev.off()
```
...
```{r}
G_rebounds <-  plot_rebound(platcov_dat_analysis, platcov_dat_rebound, trt_colors)
G_rebounds
```
...
rebound_heatmap <- ggplot(platcov_dat_rebound %>% filter(rebound == T), 
