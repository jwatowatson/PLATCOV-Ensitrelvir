...
```{r}
quant <- quantile(Baseline_data$Study_time, seq(0,1,0.2))

platcov_dat <- platcov_dat %>% 
  mutate(period_id = cut(Study_time, breaks = quant, include.lowest = TRUE, labels = FALSE)) 
```
...
```{r}
prop.test(rebound_fq$x, rebound_fq$n)
```

```{r}
pairwise.prop.test(rebound_fq$x, rebound_fq$n, p.adjust.method = "none")
```
